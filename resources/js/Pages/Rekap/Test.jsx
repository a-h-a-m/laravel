...
export default function Index({ session }) {
    ...
    const tampilkan = useEffect(() => {
        if(selesai) {
            ...
            alert('Proses percobaan selesai. Mohon dicek di whatsapp apakah pesan berhasil');
            setTerkirim(count);
            // router.post('/selesai', {
            ...
            <SectionBody>
                <Card>
                    ...
                    Silakan memilih Bulan dan Tahun Pelajaran lalu klik tombol 'Pilih'
                        ...
                        {!prosesKirim && <SearchForm word={'nama'} year={tahun} month={bulan} test={true} />}
                        <Table>
                            <BorderedTable>
                                <THead columns={['Nama', 'Ziyadah', 'Total', 'Juz', 'No Whatsapp', 'Status']} />
                                <TBody data={data ? data : dataSantri} k={'key'} from={1} keys={['nama', 'ziyadah', 'total', 'juz', 'wa', 'status']} />
                            </BorderedTable>                            
                            
                            {!selesai && !terkirimAll &&
                            (<button className="btn btn-info" onClick={sendAll}>Kirim pesan</button>)}
                        </Table>
                        </>
                    }
                    </CardBody>
